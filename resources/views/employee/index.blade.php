        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="pt-6 px-6 text-gray-900">
                    <x-primary-href :href="route('employee.create')">
                        {{ __('Create Employee') }}
                    </x-primary-href>
                </div>
                    ...
                    <table class="min-w-full bg-white border border-gray-200">
                        <thead>
                            <tr class="w-full bg-gray-100 border-b">
                                ...
                                <th class="px-6 py-3 text-left text-sm font-medium text-gray-600">Department</th>
                            ...
                            @forelse ($employees as $employee)
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-700">
                                        {{ $employee->salary === 0 ? "-" : number_format($employee->salary, 0, '.', '.') }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $employee->department->name }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-700">
                                        <x-primary-href :href="route('employee.edit', $employee->id)" class="mb-2">
                                            {{ __('Edit') }}
                                        </x-primary-href>
                                        <form action="{{ route('employee.destroy', $employee->id) }}" method="POST" class="form-delete">
                                            @csrf
                                            @method('DELETE')
